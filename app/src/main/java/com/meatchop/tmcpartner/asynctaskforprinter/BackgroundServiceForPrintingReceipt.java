...
public class BackgroundServiceForPrintingReceipt  extends Service {
    ...
    List<Printer_POJO_Class> cart_Item_List_ForPrint = new ArrayList<>() ;

    HashMap<String, Modal_NewOrderItems> cartItem_hashmap = new HashMap<>();
    boolean isPrintedSecondTime = false;
    private boolean isRunning;
    Context context;
     int no_of_timesPrintCalled = 0 , no_of_timesdoWhilePrintCalled = 0 ;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        modal_forPrintReceipt.setSelected_Address_modal(intent.getParcelableExtra("selectedAddress"));

        no_of_timesPrintCalled = 0;
            printRecipt();
    ...
    public class MyBinder extends Binder {
        public BackgroundServiceForPrintingReceipt getService() {
            return BackgroundServiceForPrintingReceipt.this;
        }
    }
    ...
    private class MyTask extends AsyncTask<Void, Void, Void> {
        ...
        //   Log.v("printer status after closeserivce 1 ",String.valueOf(PrinterFunctions.CheckStatus(portName,portSettings,1)));

            PrinterFunctions.ClosePort(portName);
           // Log.v("printer status after closeserivce 1 ",String.valueOf(PrinterFunctions.CheckStatus(portName,portSettings,1)));
         //   Log.v("printer status after closeserivce 2 ",String.valueOf(PrinterFunctions.CheckStatus(portName,portSettings,2)));
          //  Log.v("printer status after closeserivce 3 ",String.valueOf(PrinterFunctions.CheckStatus(portName,portSettings,3)));
           // Log.v("printer status after closeserivce 4 ",String.valueOf(PrinterFunctions.CheckStatus(portName,portSettings,4)));
    ...
    public void printRecipt() {
        try {
            ...
            do{
                PrinterFunctions.PortDiscovery(portName, portSettings);
                PrinterFunctions.OpenPort(portName, portSettings);
                Log.v("printer in do 255 1  ", String.valueOf(PrinterFunctions.CheckStatus(portName, portSettings, 1)));
                Log.v("printer in do 255 1 ", String.valueOf(PrinterFunctions.CheckStatus(portName, portSettings, 1)));
                no_of_timesdoWhilePrintCalled = no_of_timesdoWhilePrintCalled+1;


                if(no_of_timesdoWhilePrintCalled > 30){



                    Intent intentttt = new Intent("receiverForCompletePrintingtheReceipt");
                    intentttt.putExtra("message", "Printer Not Found");
                    sendBroadcast(intentttt);
                    break;
            ...
            while (String.valueOf(PrinterFunctions.CheckStatus(portName, portSettings, 1)).equals("255"));

            if (String.valueOf(PrinterFunctions.CheckStatus(portName, portSettings, 1)).equals("255")) {
                if (no_of_timesPrintCalled < 5) {
                    Log.v("printer before is in 255 1  ", String.valueOf(PrinterFunctions.CheckStatus(portName, portSettings, 1)));
                    Log.v("printer after is in 255 1 ", String.valueOf(PrinterFunctions.CheckStatus(portName, portSettings, 1)));
                    PrinterFunctions.PortDiscovery(portName, portSettings);
                    PrinterFunctions.OpenPort(portName, portSettings);

                    no_of_timesPrintCalled = no_of_timesPrintCalled + 1;
                    printRecipt();
                } else {
                    Intent intentttt = new Intent("receiverForCompletePrintingtheReceipt");
                    intentttt.putExtra("message", "Printer Not Found");
                ...
                try {
                    ...
                    PrinterFunctions.SelectPrintMode(portName, portSettings, 0);
                //    Log.v("printer status before close 1 ", String.valueOf(PrinterFunctions.CheckStatus(portName, portSettings, 1)));
                 //   Log.v("printer status before close 2 ", String.valueOf(PrinterFunctions.CheckStatus(portName, portSettings, 2)));
                 //   Log.v("printer status before close 3 ", String.valueOf(PrinterFunctions.CheckStatus(portName, portSettings, 3)));
                 //   Log.v("printer status before close 4 ", String.valueOf(PrinterFunctions.CheckStatus(portName, portSettings, 4)));
                ...
                if (String.valueOf(PrinterFunctions.CheckStatus(portName, portSettings, 1)).equals("255")) {
                 //   Log.v("printer before is in 255 2  ", String.valueOf(PrinterFunctions.CheckStatus(portName, portSettings, 1)));
                 //   Log.v("printer after is in 255 2 ", String.valueOf(PrinterFunctions.CheckStatus(portName, portSettings, 1)));
                    PrinterFunctions.PortDiscovery(portName, portSettings);
                    PrinterFunctions.OpenPort(portName, portSettings);
                     ...
                     */
                        ...
                        //    Printer_POJO_Class[] Printer_POJO_ClassArray = new Printer_POJO_Class[0];
                        double oldSavedAmount = 0;
                        // String CouponDiscount = "0";
                        String Gstt = "", subtotall = "", quantity = "", price = "", priceperkg_unitprice = "", weight = "", netweight = "",
                                priceSuffix = "", pricetypeforpos = "";
                        double gst_double = 0, subtotal_double = 0, price_double = 0;
                        ...
                        for (int i = 0; i < cart_Item_List.size(); i++) {
                            ...
                            if (String.valueOf(pricetypeforpos).toUpperCase().equals("TMCPRICE")) {
                                ...
                                if(priceperkg_unitprice.equals(" ") || priceperkg_unitprice.equals("  ") || priceperkg_unitprice.equals("") || priceperkg_unitprice.equals("0") || priceperkg_unitprice.equals("0.00") || priceperkg_unitprice.equals("0.0") || priceperkg_unitprice.equals("0.000")){
                                    try {
                                        priceperkg_unitprice = String.valueOf(modal_newOrderItems.getItemPrice_quantityBased());
                                        if (priceperkg_unitprice.equals("null")) {
                                            priceperkg_unitprice = "  ";
                                        }
                                    } catch (Exception e) {
                            ...
                            priceSuffix = priceSuffix.replaceAll(" ", "");
                            ...
                            Printer_POJO_Class printer_POJO_ClassArrayModal = new Printer_POJO_Class();


                            printer_POJO_ClassArrayModal.setGrossWeight("");
                            printer_POJO_ClassArrayModal.setQuantity(quantity);
                            printer_POJO_ClassArrayModal.setOrderID(modal_forPrintReceipt.getOrderid());
                            printer_POJO_ClassArrayModal.setItemName(itemName);
                            printer_POJO_ClassArrayModal.setItemWeight(weight);
                            printer_POJO_ClassArrayModal.setItemRate(price);
                            printer_POJO_ClassArrayModal.setDiscount("0.00");
                            printer_POJO_ClassArrayModal.setGST(Gstt);
                            printer_POJO_ClassArrayModal.setSubTotal(subtotall);
                            printer_POJO_ClassArrayModal.setCutname("");
                            printer_POJO_ClassArrayModal.setPriceperkg_unitprice(priceperkg_unitprice);
                            printer_POJO_ClassArrayModal.setPricetypeforpos(pricetypeforpos);
                            printer_POJO_ClassArrayModal.setPriceSuffix(priceSuffix);


                            cart_Item_List_ForPrint.add(printer_POJO_ClassArrayModal);
                        ...
                        } else if ((modal_forPrintReceipt.getVendorKey().equals("vendor_6"))) {
                        ...
                        PrinterFunctions.SetLineSpacing(portName, portSettings, 80);
                        PrinterFunctions.SelectCharacterFont(portName, portSettings, 0);
                        PrinterFunctions.PrintText(portName, portSettings, 0, 0, 0, 0, 0, 0, 0, 1, String.valueOf(modal_forPrintReceipt.getOrderplacedTime()) + "\n");
                        ...
                        PrinterFunctions.SetLineSpacing(portName, portSettings, 80);
                        PrinterFunctions.SelectCharacterFont(portName, portSettings, 0);
                        PrinterFunctions.PrintText(portName, portSettings, 0, 0, 1, 0, 0, 0, 30, 0, "ITEM NAME * QTY           PRICEPER KG/PC" + "\n");
                        ...
                        //// paste here the itemdesp loop


                        for (int i = 0; i < cart_Item_List_ForPrint.size(); i++) {


                            Printer_POJO_Class printer_pojo_class = cart_Item_List_ForPrint.get(i);
                            PrinterFunctions.SetLineSpacing(portName, portSettings, 80);
                            PrinterFunctions.SelectCharacterFont(portName, portSettings, 0);
                            String itemrate, Gst, subtotal;
                            itemrate = "Rs." + printer_pojo_class.getItemRate();
                            Gst = "Rs." + printer_pojo_class.getGST();
                            subtotal = "Rs." + printer_pojo_class.getSubTotal();
                            ...
                            StringBuilder itemnameWithWeightAndQunatity = new StringBuilder();
                            itemnameWithWeightAndQunatity = new StringBuilder(printer_pojo_class.getItemName() + " * " + printer_pojo_class.getItemWeight() + "(" + printer_pojo_class.getQuantity() + ") ");
                            int lengthofitemnameWithWeightAndQunatity = 0, no_of_spacesallowed = 0;
                            lengthofitemnameWithWeightAndQunatity = itemnameWithWeightAndQunatity.length();

                            if (printer_pojo_class.getPriceperkg_unitprice().length() <= 3) {
                                no_of_spacesallowed = 40 - lengthofitemnameWithWeightAndQunatity;
                            } else if (printer_pojo_class.getPriceperkg_unitprice().length() == 4) {

                                no_of_spacesallowed = 39 - lengthofitemnameWithWeightAndQunatity;
                            } else {
                                no_of_spacesallowed = 38 - lengthofitemnameWithWeightAndQunatity;
                            }


                            String tmcpriceperkg_or_pc = "";
                            if (String.valueOf(printer_pojo_class.getPricetypeforpos()).toUpperCase().equals("TMCPRICE")) {

                                tmcpriceperkg_or_pc = "[Rs." + printer_pojo_class.getPriceperkg_unitprice() + " -" + printer_pojo_class.getPriceSuffix() + "]";

                            } else {

                                tmcpriceperkg_or_pc = "[Rs." + printer_pojo_class.getPriceperkg_unitprice() + "/Kg]";

                            }

                            int tmcpriceperkg_or_pcLength = 0;
                            tmcpriceperkg_or_pcLength = tmcpriceperkg_or_pc.length();

                            no_of_spacesallowed = no_of_spacesallowed - tmcpriceperkg_or_pcLength;
                            try {
                                for (int count = 0; count < no_of_spacesallowed; count++) {
                                    itemnameWithWeightAndQunatity.append(" ");
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                            }

                            PrinterFunctions.PrintText(portName, portSettings, 0, 0, 0, 0, 0, 0, 30, 0, itemnameWithWeightAndQunatity + tmcpriceperkg_or_pc + "\n");
            ...
            for (int i = 0; i < Printer_POJO_ClassArray.length; i++) {
                ...
                StringBuilder itemnameWithWeightAndQunatity = new StringBuilder();
                itemnameWithWeightAndQunatity = new StringBuilder(Printer_POJO_ClassArray[i].getItemName() + " * " + Printer_POJO_ClassArray[i].getItemWeight() + "(" + Printer_POJO_ClassArray[i].getQuantity() + ") ");
                int lengthofitemnameWithWeightAndQunatity =0 , no_of_spacesallowed =0;
                lengthofitemnameWithWeightAndQunatity = itemnameWithWeightAndQunatity.length();

                if(Printer_POJO_ClassArray[i].getPriceperkg_unitprice().length()<=3){
                    no_of_spacesallowed = 40 - lengthofitemnameWithWeightAndQunatity;
                }
                else if(Printer_POJO_ClassArray[i].getPriceperkg_unitprice().length()==4) {
                ...
                String tmcpriceperkg_or_pc = "";
                if(String.valueOf(Printer_POJO_ClassArray[i].getPricetypeforpos()).toUpperCase().equals("TMCPRICE")) {

                    tmcpriceperkg_or_pc = "[Rs." +Printer_POJO_ClassArray[i].getPriceperkg_unitprice()+" -"+Printer_POJO_ClassArray[i].getPriceSuffix()+"]";

                }
                else {

                    tmcpriceperkg_or_pc = "[Rs." +Printer_POJO_ClassArray[i].getPriceperkg_unitprice()+"/Kg]";
 ...
 */
                        ...
                        if (isPrintedSecondTime) {
                            ...
                            //  context = this;
                            //   MyAsyncTaskForPrintingReceipttt asyncTaskForPrintingReceipt = new MyAsyncTaskForPrintingReceipttt();
                            ...
                            // Intent newIntent = new Intent(this, BackgroundServiceForPrintingReceipt.class);
    ...
    public class MyAsyncTaskForPrintingReceipttt extends AsyncTask<Void, Void, String> {

        @Override
        protected String doInBackground(Void... voids) {


         Intent   serviceIntent = new Intent(context, BackgroundServiceForPrintingReceipt.class);

            serviceIntent.putExtra("modalData",modal_forPrintReceipt);
            serviceIntent.putExtra("hashmapdata",cartItem_hashmap);
            serviceIntent.putExtra("isCalledSecondtime", true);
