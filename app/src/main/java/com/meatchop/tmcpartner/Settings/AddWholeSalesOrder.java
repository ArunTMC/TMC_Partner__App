...
public class AddWholeSalesOrder extends AppCompatActivity {
    ...
    static Adapter_AddWholeSalesOrdersRecyclerview adapter_addWholeSalesOrdersRecyclerview;
    ...
    EditText wholeSaleOrdersCustomermobileno;
    ...
    String portName = "USB", usermobileNo = "";
    ;
    ...
    private static final String ACTION_USB_PERMISSION = "com.android.example.USB_PERMISSION.AddWholeSalesOrder";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_whole_sales_order);
        ...
        wholeSaleOrdersCustomermobileno = findViewById(R.id.editTextPhone);
        ...
        procced_to_pay_widget.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (wholeSaleOrdersCustomermobileno.getText().toString().length() == 10) {
                    ...
                    if (cart_Item_List.size() > 0 && cartItem_hashmap.size() > 0) {
                        ...
                        if ((!total_item_Rs_text_widget.getText().toString().equals("0")) && (!total_Rs_to_Pay_text_widget.getText().toString().equals("0")) &&(!total_item_Rs_text_widget.getText().toString().equals("0.0")) && (!total_Rs_to_Pay_text_widget.getText().toString().equals("0.0"))&&(!total_item_Rs_text_widget.getText().toString().equals("0.00")) && (!total_Rs_to_Pay_text_widget.getText().toString().equals("0.00"))&&(!total_item_Rs_text_widget.getText().toString().equals("")) && (!total_Rs_to_Pay_text_widget.getText().toString().equals(""))) {
                            if (checkforBarcodeInCart("empty")) {
                                AddWholeSalesOrder.cart_Item_List.remove("empty");

                                AddWholeSalesOrder.cartItem_hashmap.remove("empty");
                            }

                            if (checkAllPriceperkgItemWeightWasEdited()){
                                long sTime = System.currentTimeMillis();
                                usermobileNo = "+91"+wholeSaleOrdersCustomermobileno.getText().toString();

                                Currenttime = getDate_and_time();
                              //  PlaceOrdersinDatabaseaAndPrintRecipt("j", sTime, Currenttime, cart_Item_List);

                                runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        try {
                                            Dialog dialog = new Dialog(AddWholeSalesOrder.this);
                                ...
                                AlertDialogClass.showDialog(AddWholeSalesOrder.this, R.string.Orders_weight_Cant_be_leave_unedited);
                            ...
                            AlertDialogClass.showDialog(AddWholeSalesOrder.this, R.string.Cant_place_order);

                        }
                    } else {
                        AlertDialogClass.showDialog(AddWholeSalesOrder.this, R.string.Cart_is_empty);

                    }

                } else {
                    AlertDialogClass.showDialog(AddWholeSalesOrder.this, R.string.Enter_the_mobile_no_text);
    ...
    private boolean checkforBarcodeInCart(String itemUniquecode) {
        ...
        for(String str: AddWholeSalesOrder.cart_Item_List) {
    ...
    void CallAdapter() {
         adapter_addWholeSalesOrdersRecyclerview = new Adapter_AddWholeSalesOrdersRecyclerview(getApplicationContext(),cartItem_hashmap, MenuItem, AddWholeSalesOrder.this);
        adapter_addWholeSalesOrdersRecyclerview.setHandler(newHandler());
        recyclerView.setLayoutManager(new LinearLayoutManager(getApplicationContext()));
        int last_index=AddWholeSalesOrder.cartItem_hashmap.size()-1;

        recyclerView.setAdapter(adapter_addWholeSalesOrdersRecyclerview);
    ...
    private void PlaceOrdersinDatabaseaAndPrintRecipt(String paymentMode, long sTime, String currenttime, List<String> cart_Item_list) {
        ...
        if (ispaymentMode_Clicked) {
            ...
            if (!isOrderDetailsMethodCalled) {
                ...
                PlaceOrder_in_OrderDetails(AddWholeSalesOrder.cart_Item_List, paymentMode, sTime);
            }
    ...
    private void PlaceOrder_in_OrderDetails(List<String> cart_Item_List1, String Payment_mode, long sTime) {
        ...
        String tokenno = "";
        String userid = "";
        String ordertype = Constants.WholeSaleOrder;
...
/*
            if(isinventorycheck) {
                ...
                try {
                    ...
                    quantityDouble = Double.parseDouble(quantity1);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                        ...
                        try {
                            if(printerType_sharedPreference.equals(Constants.USB_PrinterType)){
                                ...
                                if(i != -1){
                                    ...
                                    new TMCAlertDialogClass(AddWholeSalesOrder.this, R.string.app_name, R.string.OrderPlaced_Printer_is_Disconnected,
                                            R.string.OK_Text,R.string.Empty_Text,
                                            new TMCAlertDialogClass.AlertListener() {
                                                @Override
                                                public void onYes() {
                                                    AddWholeSalesOrder. cart_Item_List.clear();

                                                    AddWholeSalesOrder. cartItem_hashmap.clear();
                                                    ...
                                                    wholeSaleOrdersCustomermobileno.setText("");
                                        ...
                                        new TMCAlertDialogClass.AlertListener() {
                                            ...
                                            public void onYes() {
                                               AddWholeSalesOrder. cart_Item_List.clear();
                                                ...
                                                showProgressBar(false);
                                                //Toast.makeText(AddWholeSalesOrder.this,"Please Generate Token Number Again",Toast.LENGTH_SHORT).show();
                                            }
 ...
 */
    ...
    private void PrintReciptForNewItemUsingUSBPrinter(String userMobile, String tokenno, String itemTotalwithoutGst, String taxAmount, String payableAmount, String orderid, List<String> cart_item_list, HashMap<String, Modal_NewOrderItems> cartItem_hashmap, String payment_mode, String finalCouponDiscountAmount, String ordertype) {
        ...
        UsbConnection usbConnection = UsbPrintersConnectionsLocal.selectFirstConnected(AddWholeSalesOrder.this);
        UsbManager usbManager = (UsbManager) AddWholeSalesOrder.this.getSystemService(Context.USB_SERVICE);

        if (usbConnection == null || usbManager == null) {
            showProgressBar(false);

          /*  new AlertDialog.Builder(AddWholeSalesOrder.this)
           ...
           */
            ...
            new TMCAlertDialogClass(AddWholeSalesOrder.this, R.string.app_name, R.string.ReConnect_Instruction,
        ...
        PendingIntent permissionIntent = PendingIntent.getBroadcast(
                AddWholeSalesOrder.this,
                0,
                new Intent(ACTION_USB_PERMISSION),
                android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S ? PendingIntent.FLAG_MUTABLE : 0
        );
        IntentFilter filter = new IntentFilter(ACTION_USB_PERMISSION);
        filter.addCategory("Add Swiggy Orders");
        registerReceiver(usbReceiver_WholeSalenewItem, filter);
    ...
    private final BroadcastReceiver usbReceiver_WholeSalenewItem = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            Set<String> category = intent.getCategories();
            if (ACTION_USB_PERMISSION.equals(action)) {
                synchronized (AddWholeSalesOrder.this) {
                    UsbManager usbManager = (UsbManager) AddWholeSalesOrder.this.getSystemService(Context.USB_SERVICE);
                        ...
                        if (usbManager != null && usbDevice != null) {
                            new AsyncUsbEscPosPrint(
                                ...
                                public void onError(AsyncEscPosPrinter asyncEscPosPrinter, int codeException) {
                                    ...
                                    try{
                                        unregisterReceiver(usbReceiver_WholeSalenewItem);
                                    }
                                    catch (Exception e){
                                        e.printStackTrace();
                                    }
                                ...
                                public void onSuccess(AsyncEscPosPrinter asyncEscPosPrinter) {
                                    ...
                                    if (!isPrintedSecondTime) {
                                        ...
                                        new TMCAlertDialogClass(AddWholeSalesOrder.this, R.string.app_name, R.string.RePrint_Instruction,
 ...
 */
        //    text_to_Print = text_to_Print + "[L]" + payableAmount + " \n";
            ...
            Toast.makeText(AddWholeSalesOrder.this, "Size of Cart is not matched", Toast.LENGTH_SHORT).show();
        }
    ...
    private void uploadMenuAvailabilityStatusTranscationinDB(String userPhoneNumber, String menuItemName, boolean availability, String menuItemSubCtgykey, String vendorkey, String dateandtime, String menuItemKey, String message, String menuItemStockAvlDetailskey, boolean allowNegative, String itemStockAvlDetailskey) {
        ...
        // Make the request
        Volley.newRequestQueue(AddWholeSalesOrder.this).add(jsonObjectRequest);
                ...
                public void run() {
                    ...
                    JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, Constants.api_getListofStockOutGoingDetailsForStockIncmgKey + stockIncomingKey_avlDetails, null,
                        ...
                        public void onErrorResponse(@NonNull VolleyError error) {
                            ...
                            Toast.makeText(AddWholeSalesOrder.this, "Error in General App Data code :  " + errorCode, Toast.LENGTH_LONG).show();
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, Constants.api_addEntry_StockTransactionHistory
            ...
            public void onResponse(@NonNull JSONObject response) {
                ...
                try {
                    ...
                    // showProgressBar(false);
                    Toast.makeText(AddWholeSalesOrder.this,"Failed to Add Data in Stock Outgoing table",Toast.LENGTH_LONG).show();
                    e.printStackTrace();
                }
        ...
        if(changeItemAvailability){
            ...
            JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, Constants.api_updateMenuItemDetails,
                ...
                public void onErrorResponse(@NonNull VolleyError error) {
                    ...
                    //Log.d(Constants.TAG, "Error: " + error.toString());
                    showProgressBar(false);
                    Toast.makeText(AddWholeSalesOrder.this,"Failed to change express delivery slot status inDelivery slot details",Toast.LENGTH_LONG).show();
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, api_Update_MenuItemStockAvlDetails
            ...
            public void onResponse(@NonNull JSONObject response) {
                ...
                try {
                    ...
                    // showProgressBar(false);
                    Toast.makeText(AddWholeSalesOrder.this,"Failed to change express delivery slot status in Delivery slots",Toast.LENGTH_LONG).show();
                    e.printStackTrace();
                }
    ...
    private void savedMenuIteminSharedPrefrences(List<Modal_MenuItem_Settings> menuItem, int iterator_menuitemStockAvlDetails) {
        ...
        editor.apply();
        try {
            adapter_addWholeSalesOrdersRecyclerview.notifyDataSetChanged();
            adapter_addWholeSalesOrdersRecyclerview.notify();
            adapter_addWholeSalesOrdersRecyclerview.notifyItemChanged(iterator_menuitemStockAvlDetails);

            adapter_addWholeSalesOrdersRecyclerview.notifyAll();
        }
        catch (Exception e){
            e.printStackTrace();
        }
